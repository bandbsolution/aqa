...
function checkEmailSubscriptionsByDefault() {
    cy.iframe('#accSettingsPage').find('[role="region"]').eq(1).find('input').eq(0).should('be.checked');
    cy.iframe('#accSettingsPage').find('[role="region"]').eq(1).find('input').eq(1).should('be.checked');
    cy.iframe('#accSettingsPage').find('[role="region"]').eq(1).find('input').eq(2).should('be.checked');
    cy.iframe('#accSettingsPage').find('[role="region"]').eq(1).find('input').eq(3).should('not.be.checked');
}
...
function turnOffEmailSubscriptions() {
    ...
    cy.iframe('#accSettingsPage').find('[role="region"]').eq(1).find('input').eq(1).should('not.be.checked');
    cy.iframe('#accSettingsPage').find('[role="region"]').eq(1).find('input').eq(2).should('not.be.checked');
}
...
describe('get emails: change status of order, leave feedback, new order', () => {
    ...
    it('settings - default values for email subscribe for two accounts (activated)', () => {
        cy.login(emailAddress, password);
        authModals.choseMenuInSettings(SettingsMenu.Notifications, SettingsMenuBlocks.EmailSubscription);
        checkEmailSubscriptionsByDefault();
        authModals.navigateToMenuItem(ProfileActions.LOGOUT);
        cy.login(secondEmailAddress, password);
        authModals.choseMenuInSettings(SettingsMenu.Notifications, SettingsMenuBlocks.EmailSubscription);
        checkEmailSubscriptionsByDefault();
